...
class MIDIKeyboard(VirtualInstrument):
    def __init__(self, keys: int, start_key: str):
        from_ = dropwhile(
            lambda elem: elem[0] != start_key,
            self.notes.items()
        )
        to_ = islice(from_, 0, keys)
        self.notes: dict[str, float] = dict(to_)


class Guitar(VirtualInstrument):
    def __init__(self, frets: int, tuning: list[str]):
        self.notes: dict[int, dict[str, float]] = {
            n: dict(islice(dropwhile(
                lambda elem: elem[0] != note, 
                self.notes.items()
            ), 0, frets))
            for n, note in enumerate(tuning, 1)
        }


# >>> VirtualInstrument.notes
...
# >>> axiom49 = MIDIKeyboard(49, 'C3')
# >>> axiom49.notes
# {'C3': 130.81, 'C#3': 138.59, 'D3': 146.83, 'D#3': 155.56, 'E3': 164.81, 'F3': 174.61, 'F#3': 185.0, 'G3': 196.0, 'G#3': 207.65, 'A3': 220.0, 'A#3': 233.08, 'B3': 246.94, 'C4': 261.63, 'C#4': 277.18, 'D4': 293.66, 'D#4': 311.13, 'E4': 329.63, 'F4': 349.23, 'F#4': 369.99, 'G4': 392.0, 'G#4': 415.3, 'A4': 440.0, 'A#4': 466.16, 'B4': 493.88, 'C5': 523.25, 'C#5': 554.37, 'D5': 587.33, 'D#5': 622.25, 'E5': 659.25, 'F5': 698.46, 'F#5': 739.99, 'G5': 783.99, 'G#5': 830.61, 'A5': 880.0, 'A#5': 932.33, 'B5': 987.77, 'C6': 1046.5, 'C#6': 1108.73, 'D6': 1174.66, 'D#6': 1244.51, 'E6': 1318.51, 'F6': 1396.91, 'F#6': 1479.98, 'G6': 1567.98, 'G#6': 1661.22, 'A6': 1760.0, 'A#6': 1864.66, 'B6': 1975.53, 'C7': 2093.0}


# >>> sigma = Guitar(21, ['E3', 'B2', 'G2', 'D2', 'A1', 'E1'])
# >>> 
# >>> from pprint import pprint
# >>>
# >>> pprint(sigma.notes, sort_dicts=False)
# {1: {'E3': 164.81,
#      'F3': 174.61,
#      'F#3': 185.0,
#      'G3': 196.0,
